...
- P03 종목 상세를 연결했다. P01 순위와 P02 검색 결과에서 상세로 이동하며 브라우저·플랫폼 뒤로가기 이력을 유지한다. 종목 요약, 예시 가격 차트, 기간 선택, 가상 매수일 선택 상태, 관련 종목 탭·상세 이동, safe area를 반영한 하단 CTA가 있다. 실제 시세·업종 데이터는 아직 연결하지 않았다.
- P04 가상 매수 입력을 P03 하단 CTA에 연결했다. 날짜, 예수금·주식 정리·혼합 자금 원천, 예시 보유 종목의 보유·매도 수량, 투자 비중·직접 입력, 가용 자금과 거래 요약, 자금 기록 저장 선택, 입력 오류 안내, safe area 하단 계산 버튼이 한 화면에 있다. 입력 초안은 화면을 오가는 동안 유지한다. 계산식은 화면 확인용 예시이며 실제 시세·기업행동·서버 저장은 아직 연결하지 않았다.
- P05 계산 결과를 P04 계산 버튼에 연결했다. 입력값으로 시작 자산·현재 예상 잔고·유지 가정·선택 차이를 계산하고, 잔고 구성, 접을 수 있는 계산 조건, 조건 변경, 최신 재평가 안내, 관련 종목 이동을 제공한다. 복기 메모는 이 기기의 `localStorage`에 저장한다. 결과 화면은 예시 가격과 서버 미저장 상태를 명시하며 실제 시세·기업행동·계정 기록은 아직 연결하지 않았다.
...
1. `화면설계.md` 8절을 기준으로 P06 내 기록을 `invest-if/src/`에 추가하고 하단 ‘내 기록’ 메뉴를 연결한다. 계산 결과 스냅샷의 로컬 저장, 검색·자금 원천 필터, 기록 카드·빈 상태·상세 재진입을 먼저 구현하고 계정 저장과 구분한다.
