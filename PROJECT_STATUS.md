...
- P04 가상 매수 입력을 P03 하단 CTA에 연결했다. 날짜, 예수금·주식 정리·혼합 자금 원천, 예시 보유 종목의 보유·매도 수량, 투자 비중·직접 입력, 가용 자금과 거래 요약, 자금 기록 저장 선택, 입력 오류 안내, safe area 하단 계산 버튼이 한 화면에 있다. 입력 초안은 화면을 오가는 동안 유지한다. 계산식은 화면 확인용 예시이며 실제 시세·기업행동·서버 저장과 P05 결과 화면은 아직 연결하지 않았다.
...
1. `화면설계.md` 7절과 `MVP개발명세.md` 4.4·5절을 기준으로 P05 계산 결과·기록 상세를 `invest-if/src/`에 추가하고 P04 계산 버튼을 연결한다. 먼저 현재 예시 입력값으로 유지/변경 잔고 비교와 메모·저장 상태를 구현하고, 실제 시세·기업행동 연결과 구분한다.
