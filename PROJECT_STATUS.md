...
- 루트의 기존 Vite 프로토타입은 삭제하지 않았다. `invest-if/src/App.tsx`와 `App.css`에 P01 둘러보기 홈을 구현했다. 검색 진입, 거래량/시가총액 순위 전환, 예시 종목 5개/더 보기, 둘러보기/내 기록 하단 메뉴와 safe area 처리가 들어갔다.
...
- P06 내 기록을 하단 메뉴에 연결했다. 계산 성공 시 입력값과 결과 스냅샷을 브라우저 `localStorage`에 자동 저장하며, 종목 검색, 전체·예수금·주식 정리·혼합 필터, 최신순 카드, 메모 미리보기, 빈 상태, 결과 상세 재진입을 제공한다. 저장 실패와 계정 미동기화 상태를 화면에 구분해 표시한다.
