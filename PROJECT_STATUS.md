...
- 루트의 기존 Vite 프로토타입은 삭제하지 않았다. 첫 이전 단계로 `invest-if/src/App.tsx`와 `App.css`에 P01 둘러보기 홈을 구현했다. 검색 진입, 거래량/시가총액 순위 전환, 예시 종목 5개/더 보기, 둘러보기/내 기록 하단 메뉴와 safe area 처리가 들어갔다. 아직 옮기지 않은 검색·상세·기록 화면은 현재 화면 안에서 다음 단계 안내만 표시한다.
...
1. `화면설계.md` 5절을 기준으로 P03 종목 상세를 `invest-if/src/`에 추가하고 P01 순위·P02 검색 결과를 상세 화면으로 연결한다. 차트와 관련 종목 탭, 선택 상태 보존, 하단 고정 CTA를 함께 구현한다.
